...
function FusedCouncil:OnEnable()
	...
	self:RegisterEvent("PLAYER_ENTERING_WORLD", "enterInstance");
	self:RegisterEvent("PARTY_LOOT_METHOD_CHANGED", "onLootChange");
	----------PopUp setups --------------
	-------------------------------------
...
function FusedCouncil:onLootChange()
	if GetLootMethod() == "master" then
		LibDialog:Spawn("FC_CONFIRM_USAGE");	
	end
end
function FusedCouncil:enterInstance()
	local isInstance, instanceType = IsInInstance();
	if isInstance and instanceType == "raid" and UnitIsGroupLeader("player") and not usingAddon then
		--toast to change to ML
		LibDialog:Spawn("FC_CONFIRM_USAGE");
	end
...
function FusedCouncil:prossessLootedBody()
local MLItems = {};
...
-- do we have any items that need to be MLed?
	...
	if #MLItems > 0 then
		...
		for i=1, #MLItems do
			local item = self:findItem(MLItems[i], itemBank);
			if item then
				item["count"] = item["count"] + 1;
			else
				table.insert(itemBank, FusedCouncil:createItem(MLItems[i]));
			end
		end
		...
		for i=1, GetNumGroupMembers() do
			...
			local name = select(1, GetMasterLootCandidate(1,i));--GetRaidRosterInfo(i));
			if name then
				print(name)
...
function FusedCouncil:lootOpenedHandler(eventName, ...)

local lootMethod, isML = GetLootMethod();
local lootBody = UnitName("target");

	if lootMethod =="master" and isML == 0 then
		if sessionID ~= "" then
			-- if we do not have a fresh sessionID
			if lootBody ~= bodyLooted then
				-- we have a new body being looted
				FusedCouncil:prossessLootedBody();
...
function FusedCouncil:test()
	mainCouncilFrame:Show();
	testing = true;
	ML = UnitName("player");
	sessionID = tostring(math.floor(GetTime())) .. UnitName("player");
	local itemList = {GetInventoryItemLink("player",16),
	...
	for i=1, #itemList do
		local item = self:findItem(itemList[i], itemBank);
		if item then
			item["count"] = item["count"] + 1;
		else
			table.insert(itemBank, FusedCouncil:createItem(itemList[i]));
		end
	end
	
	-- get a list of people that we should expect to get the loot list
	for i=1, GetNumGroupMembers() do
		local name = select(1, GetMasterLootCandidate(i));--GetRaidRosterInfo(i));
		table.insert(itemBankRecivers, name);
	end
	mainCouncilFrame:Show();
	self:sendItemBank();
	...
	if prefix == addonPrefix then
		...
		if success then
			if payload["cmd"] == "itemBank" then
				self:dbug("recived itemBank");
				self:dbug("sessionID is ".. payload["sessionID"])
				if sessionID == "" then
					self:dbug("new Session")
			...
			elseif payload["cmd"] == "ack" then
				...
				elseif payload["type"] == "unvote" then
...
function FusedCouncil:isCouncilMember()
	if localOptions then
		for i=1, #localOptions["lootCouncilMembers"] do			
			if UnitName("player") == localOptions["lootCouncilMembers"][i] then
				return true;
			end
		end
	else
...
function FusedCouncil:sendItemBank()
	local payload = {cmd="itemBank", ML= ML, itemBank = itemBank, sessionID = sessionID, options = dbProfile.options, testing = testing};
	local serPayload = FusedCouncil:Serialize(payload);
	self:SendCommMessage(addonPrefix, serPayload, "RAID");
	self:dbug("sent itemBank sessid " .. sessionID);
